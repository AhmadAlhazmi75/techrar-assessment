...
const ChatButton = ({disabled}: {disabled: boolean}) => {
  return (
      <WetPaintButton disabled={disabled} />
  );
};

const WetPaintButton = ({disabled}: {disabled: boolean}) => {
  return (
    <button type="submit" className="group relative rounded bg-violet-500 px-4 py-2.5 font-semibold text-white transition-colors hover:bg-violet-600" disabled={disabled}>
      Send
